...
describe("/api/posts", () => {
  ...
  describe("GET /", () => {
    ...
    it("should return posts by filter price", async () => {
      ...
      const res = await request(server).get(str).set("x-access-token", token);
      expect(res.status).toBe(200);
    });
    it("should return posts by filter address", async () => {
      const token = await generateAdminToken();
      let str = encodeURI(
        "/api/posts?filter=address&city=Đà Nẵng&district=Cẩm Lệ&ward=Hòa An&street=Đoàn Văn"
      );
      const res = await request(server).get(str).set("x-access-token", token);
      expect(res.status).toBe(200);
    });
  ...
  describe("POST /", () => {
    it("should return post if create successful", async () => {
      ...
      const res = await request(server)
        ...
        .attach(
          "images",
          path.resolve(__dirname, "C:/Users/HungDV/Downloads/house13.jpg")
        )
        .attach(
          "images",
          path.resolve(__dirname, "C:/Users/HungDV/Downloads/house12.jpg")
        );
      expect(res.status).toBe(200);
    });
    it("should return 401 if address has already register", async () => {
      ...
      const res = await request(server)
        ...
        .attach(
          "images",
          path.resolve(__dirname, "C:/Users/HungDV/Downloads/house11.jpg")
        );
...
describe("/api/posts/:id", () => {
  ...
  describe("DELETE /", () => {
    ...
    it("should return 403 if not permission", async () => {
      ...
      const res = await request(server).delete("/api/posts/89");
      expect(res.status).toBe(403);
    });
  });
  ...
  describe("GET /posts/:id/images", () => {
    ...
    it("should return 403 if not permission", async () => {
      const res = await request(server).get("/api/posts/80/images");
      expect(res.status).toBe(403);
    });
  });
});
